...
export default function AdminGuard({ children, requiredRole = 'author' }: Props) {
  ...
  if (!isAuthenticated) return (
    <div className="flex min-h-screen items-center justify-center bg-bg">
      <div className="card max-w-md rounded-2xl p-8 text-center">
        ...
        <p className="mb-6 text-sm text-text-secondary">Please log in to access the admin panel.</p>
  ...
  if (!hasPermission(requiredRole)) return (
    <div className="flex min-h-screen items-center justify-center bg-bg">
      <div className="card max-w-md rounded-2xl p-8 text-center">
        <h2 className="mb-2 text-xl font-bold text-text">Insufficient Permissions</h2>
        <p className="mb-6 text-sm text-text-secondary">Role "{user?.role}" cannot access this area.</p>
        <a href="/" className="btn-ghost inline-block">Return Home</a>
      </div>
    </div>
  );
