...
export default function SecurityRatePolicyForm() {
  ...
  return <div className="card space-y-5 rounded-xl p-5">
    ...
    <button className="rounded bg-indigo-600 px-4 py-2 text-white" onClick={async()=>{try{setDto(await putSecurityPolicies(dto));setStatus('策略已保存');}catch{setStatus('保存失败：请检查边界、版本和 step-up');}}}>保存限额</button>
    <div className="border-t pt-4"><label className="flex items-center gap-3"><span>注册模式</span><select value={mode.mode} onChange={async e=>{try{setMode(await putRegistrationMode(e.target.value as 'open'|'invite_only',mode.version));setStatus('注册模式已保存');}catch{setStatus('注册模式保存失败');}}} className="rounded border px-2 py-1"><option value="open">公开</option><option value="invite_only">仅邀请码</option></select></label></div>
