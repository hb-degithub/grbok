...
const cardData = [
  { color: '#120F17', title: '\u6280\u672f\u6587\u7ae0', description: '\u6df1\u5ea6\u6280\u672f\u89e3\u6790\u4e0e\u5b9e\u8df5\u5206\u4eab', label: '\u6587\u7ae0', href: '/posts' },
  { color: '#120F17', title: '\u6807\u7b7e\u7d22\u5f15', description: '\u6309\u4e3b\u9898\u6d4f\u89c8\u6240\u6709\u5185\u5bb9', label: '\u6807\u7b7e', href: '/tags' },
  { color: '#120F17', title: '\u5f52\u6863\u65f6\u95f4\u7ebf', description: '\u6309\u65f6\u95f4\u56de\u6eaf\u6bcf\u4e00\u7bc7\u8bb0\u5f55', label: '\u5f52\u6863', href: '/archive' },
  { color: '#120F17', title: '\u5173\u4e8e\u535a\u4e3b', description: '\u4e86\u89e3\u5199\u8fd9\u4e9b\u6587\u5b57\u7684\u4eba', label: '\u5173\u4e8e', href: '/about' },
  { color: '#120F17', title: '\u6700\u65b0\u5185\u5bb9', description: '\u56de\u5230\u6700\u8fd1\u53d1\u5e03\u7684\u6587\u7ae0', label: '\u6700\u65b0', href: '#latest' },
  { color: '#120F17', title: '\u767b\u5f55\u5165\u53e3', description: '\u8fdb\u5165\u8d26\u53f7\u548c\u540e\u53f0\u9875\u9762', label: '\u767b\u5f55', href: '/login' }
];
...
const ParticleCard = ({
  children, className = '', disableAnimations = false, href, style,
  ...
  return (
    <a ref={cardRef} href={href} className={`${className} magic-bento-particle-container`} style={{ ...style, position: 'relative', overflow: 'hidden' }}>
      {children}
    </a>
  );
};
...
const GlobalSpotlight = ({ gridRef, disableAnimations = false, enabled = true, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, glowColor = DEFAULT_GLOW_COLOR }) => {
  ...
  useEffect(() => {
    ...
    const refreshTargets = () => {
      cards = Array.from(gridRef.current?.querySelectorAll('.magic-bento-card') || []);
      section = gridRef.current?.closest('.magic-bento-section');
    };
    refreshTargets();
    ...
    const runFrame = () => {
      rafId = null;
      const e = lastEvent;
      if (!e || !spotlightRef.current) return;
...
const BentoCardGrid = ({ children, gridRef }) => (
  <div className="magic-bento-grid magic-bento-section" ref={gridRef}>{children}</div>
);
    ...
    <div className="magic-bento-root">
      ...
      <BentoCardGrid gridRef={gridRef}>
        {cardData.map((card, index) => {
          const baseClassName = `magic-bento-card ${textAutoHide ? 'magic-bento-card--text-autohide' : ''} ${(!shouldDisableAnimations && enableBorderGlow) ? 'magic-bento-card--border-glow' : ''} ${shouldDisableAnimations ? 'active:bg-zinc-800/50' : ''}`;
          const cardProps = { className: baseClassName, style: { '--glow-color': glowColor } };
          if (enableStars) {
            return (
              <ParticleCard key={index} {...cardProps} href={card.href} disableAnimations={shouldDisableAnimations} particleCount={particleCount} glowColor={glowColor} enableTilt={enableTilt} clickEffect={clickEffect} enableMagnetism={enableMagnetism}>
