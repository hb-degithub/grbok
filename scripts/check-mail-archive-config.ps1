...
Require-Match $wrapper '(?m)^test -f "\$RCLONE_CONFIG"\s*$' 'wrapper must require RCLONE_CONFIG to be a regular file'
Require-Match $wrapper '(?m)^test ! -L "\$RCLONE_CONFIG"\s*$' 'wrapper must reject an RCLONE_CONFIG symlink'
Require-Match $wrapper '(?m)^test "\$\(stat -c ''%u:%a'' "\$RCLONE_CONFIG"\)" = "0:600"\s*$' 'wrapper must require root-owned 0600 RCLONE_CONFIG at runtime'
